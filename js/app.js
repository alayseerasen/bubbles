...
async function createPost(){
    ...
    if(!text && !file){ toast("Добавь текст или изображение."); return; }

    let image = "";
    if(file){
        if(!file.type.startsWith("image/")){ toast("Можно загружать только изображения."); return; }
        if(file.size > 8*1024*1024){ toast("Изображение слишком большое. Максимум 8 МБ."); return; }
    ...
    if(error){
        ...
        console.error(error); toast("Не удалось опубликовать пост."); return;
    }
    toast("Пост опубликован!");
    renderFeed();
}
...
async function toggleLike(postId){
    ...
    if(index>=0) post.likes.splice(index,1); else post.likes.push(currentUserId);
...
async function addComment(postId){
    ...
    if(error){ db.comments=db.comments.filter(c=>c.id!==comment.id); console.error(error); toast("Не удалось добавить комментарий."); return; }
    renderFeed();
}
...
async function deletePost(postId){
    ...
    if(postResult.error || commentResult.error){ toast("Не удалось удалить пост."); return; }
...
async function saveProfile(){
    ...
    if(!username || !displayName){toast("Заполни имя и юзернейм.");return;}
    const taken=db.users.some(u=>u.id!==user.id && u.username===username);
    if(taken){toast("Этот юзернейм уже занят.");return;}

    const avatarFile=document.getElementById("editAvatar")?.files[0];
    const coverFile=document.getElementById("editCover")?.files[0];
    if(avatarFile){ if(avatarFile.size>5*1024*1024){toast("Аватар слишком большой. Максимум 5 МБ.");return;} user.avatar=await fileToDataURL(avatarFile); }
    if(coverFile){ if(coverFile.size>8*1024*1024){toast("Обложка слишком большая. Максимум 8 МБ.");return;} user.cover=await fileToDataURL(coverFile); }
    user.username=username; user.displayName=displayName; user.bio=bio;
    ...
    if(error){console.error(error);toast("Не удалось сохранить профиль.");return;}
    toast("Профиль обновлён."); renderApp();
}
...
async function toggleFriend(userId){
    ...
    if(index>=0){
        ...
        if(error){toast("Не удалось удалить друга.");return;}
        db.friends.splice(index,1); toast("Пользователь удалён из друзей.");
    }else{
        ...
        if(error){console.error(error);toast("Не удалось добавить друга.");return;}
        db.friends.push(f); toast("Пользователь добавлен в друзья.");
    }
    renderProfile(userId);
}
...
async function sendMessage(event,userId){
    ...
    if(error){console.error(error);toast("Не удалось отправить сообщение.");return;}
    db.messages.push(message); input.value=""; renderMessages();
    setTimeout(()=>{const box=document.getElementById("chatMessages");if(box)box.scrollTop=box.scrollHeight;},20);
}
...
async function uploadMusic(){
    ...
    if(!title){toast("Укажи название трека.");return;}
    if(!artist){toast("Укажи имя артиста.");return;}
    if(!audioFile){toast("Выбери MP3-файл.");return;}
    if(audioFile.size>MAX_MUSIC_SIZE){toast("MP3 слишком большой. Максимум — 15 МБ.");return;}
    const isMP3=audioFile.type==="audio/mpeg" || audioFile.name.toLowerCase().endsWith(".mp3");
    if(!isMP3){toast("Можно загружать только MP3.");return;}
    if(coverFile && !coverFile.type.startsWith("image/")){toast("Обложка должна быть изображением.");return;}
    if(coverFile && coverFile.size>MAX_COVER_SIZE){toast("Обложка слишком большая. Максимум 5 МБ.");return;}
    ...
    try{
        ...
        console.error(error);
        try{await sb.storage.from(MUSIC_BUCKET).remove([audioPath,coverPath].filter(Boolean));}catch{}
...
async function deleteMusic(id){
    ...
    if(!music || music.authorId!==currentUserId) return;
    if(!confirm("Удалить этот трек?")) return;
    if(currentlyPlayingMusicId===id) closeMusicPlayer();
    const {error}=await sb.from("music").delete().eq("id",id);
    if(error){console.error(error);toast("Не удалось удалить трек.");return;}
    const paths=[music.audioPath,music.coverPath].filter(Boolean);
    if(paths.length) await sb.storage.from(MUSIC_BUCKET).remove(paths);
