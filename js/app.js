...
async function toggleLike(postId) {
    ...
    // Re-read the current like list from the server right before writing,
    // so two people liking at nearly the same moment don't clobber each
    // other's like.
    const { data: fresh, error: readError } = await sb.from("posts").select("likes").eq("id", postId).maybeSingle();
    const serverLikes = !readError && Array.isArray(fresh?.likes) ? fresh.likes : (post.likes || []);

    const nextLikes = new Set(serverLikes);
    const wasLiked = nextLikes.has(currentUserId);
    if (wasLiked) nextLikes.delete(currentUserId);
    else nextLikes.add(currentUserId);
    const likesArray = [...nextLikes];

    post.likes = likesArray; // optimistic local update
    ...
    if (error) {
        ...
        if (wasLiked) likesArray.push(currentUserId); else likesArray.splice(likesArray.indexOf(currentUserId), 1);
